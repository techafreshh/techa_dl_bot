                ...
                async with session.get(url, timeout=None) as response:
                    ...
                    with open(destination, "wb") as f:
                        async for chunk in response.content.iter_chunked(self.chunk_size):
                            ...
                            if progress_callback and (current_time - last_update_time > 5 or downloaded_size == total_size):
...
async def worker(bot: Bot, queue: asyncio.Queue):
    ...
    while True:
        ...
        destination = os.path.join(settings.DOWNLOAD_DIR, f"{int(time.time())}_{filename}")
        ...
        try:
            ...
            if success:
                await bot.edit_message_text("📤 **Uploading to Telegram...**", chat_id, status_msg_id, parse_mode="Markdown")
                ...
                await bot.edit_message_text("✅ **Successfully transferred!**", chat_id, status_msg_id, parse_mode="Markdown")
            else:
                await bot.edit_message_text("❌ **Failed to download the file.**", chat_id, status_msg_id, parse_mode="Markdown")
        except Exception as e:
            logger.exception(f"Worker error: {e}")
            try:
                await bot.edit_message_text(f"❌ **Error:** {str(e)}", chat_id, status_msg_id)
            except:
                pass
        finally:
            if os.path.exists(destination):
